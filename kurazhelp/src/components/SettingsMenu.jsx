...
function SettingsMenu({ currentUser, onClose, isDarkMode, toggleDarkMode }) { // NEW: isDarkMode, toggleDarkMode props
  ...
  return (
    <div className={`absolute top-14 right-4 border rounded-md shadow-lg z-50 p-4 w-60
      ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-800'}
    `}>
      <div className="flex justify-between items-center mb-3">
        <h3 className={`font-bold ${isDarkMode ? 'text-yellow-400' : 'text-yellow-600'}`}>Settings</h3> {/* Adjusted text color */}
        <button onClick={onClose} className={`${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-800'}`}>
      ...
      <div className="mb-3">
        <label className="flex items-center justify-between text-sm cursor-pointer">
          ...
          <input
            type="checkbox"
            checked={isDarkMode} // Controlled by state
            onChange={toggleDarkMode} // Toggles state
